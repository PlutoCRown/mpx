...
import { webpack as mpxWebpack } from '../src'
...
describe('webpack react native adapter', () => {
  it('bundles .mpx into an RN JS module without Compilation patches', (done) => {
    const compiler = webpack({
      ...
      target: 'node',
      output: {
        path: path.join(exampleDir, 'dist-webpack'),
        filename: 'main.js',
        clean: true
      },
      resolve: {
      ...
      plugins: [
        mpxWebpack({ mode: 'android' })
      ]
    })
    if (!compiler) {
      done(new Error('webpack returned no compiler'))
      return
    }
    ...
    compiler.run((error, stats) => {
      ...
      if (!stats) {
        finish(new Error('webpack returned no stats'))
        return
      }
      ...
      const bundle = fs.readFileSync(path.join(exampleDir, 'dist-webpack/main.js'), 'utf8')
